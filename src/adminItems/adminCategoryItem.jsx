...
const AdminCategoryItem = ({item}) => {
    ...
    return (
      <Box bg={categoryBg} onClick={() => (dispatch(selectCategory(item)))} scrollSnapAlign={"start"} cursor={'pointer'}  border={"1px"} borderColor={categoryBorder} shadow={"sm"} transition={"ease-in-out .1s"} rounded={"sm"} _active={{transform : "scale(0.95)"}} p={"1px"}>
        ...
        <Divider minH={"2px"} border={"none"} bg={state.selectCategory? state.selectCategory?.id === item.id && state.selectCategory.name_ru === item.name_ru? dividerBg : "none" : state.categories[0]?.id === item.id? dividerBg : "none" }/>
      </Box>
    )
}
